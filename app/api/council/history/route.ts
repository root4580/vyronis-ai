...
export async function GET(request: Request) {
  try {
    ...
    const state = await getCouncilHistoryState(supabase, user.id, accountId)
    ...
    console.error("Council history GET error:", error)
    return NextResponse.json(
      { error: error instanceof Error ? error.message : "Could not load council history" },
      { status: 500 },
    )
  }
}
